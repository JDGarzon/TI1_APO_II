...
public class IcesiFilms implements Serializable{
	...
	public boolean searchFunction(int roomType, Function function) {
		int i;
		Function compareTo;
		boolean flag = false;
		if(function != null) {
			switch(roomType) {
				case 1: for(i=0;i<middleRoom.size() && !flag;i++) {
							compareTo = middleRoom.get(i).getFunction();
							if(compareTo.equals(function)) {
								flag = true;
							}
						}
						
					break;
				case 2:	for(i=0;i<miniRoom.size() && !flag;i++) {
							if(((miniRoom.get(i).getFunction()).equals(function))) {
								flag = true;
							}
						}
					break;
